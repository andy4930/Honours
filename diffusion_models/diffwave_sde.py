...
class RevVPSDE(torch.nn.Module):
    ...
    def rvpsde_fn(self, t, x, return_type='drift'):
        ...
        drift, diffusion = self.vpsde_fn(t, x)     # drift.shape([1,16000])
...
class RevDiffWave(torch.nn.Module):
    def __init__(self, args, device=None):
        ...
        #load ddpm
        model_path = args.diffwav_path
        model_config = args.ddpm_config
        model = create_diffwave_model(args=args, model_path=model_path, config_path=model_config, reverse_timestep=args.t)
    ...
    def audio_editing_sample(self, audio):
        ...
        for it in range(self.args.sample_step):
            if self.args.diffusion_type == 'sde':
            ...
            elif self.args.diffusion_type == 'ddpm':

                x_ddpm_0 = self.model(x0)
                x0 = x_ddpm_0
            
            else:
                raise NotImplementedError(f'Unknown diffusion-based defense: {self.args.defense_type}!')
